...
def process_matlab_txt(root_path, raw_path, processed_path) -> None:
    ...
    for i, j in raw_files_list_enumerated:
        ...
        if df[:, 1:1444:2].shape[0] <= 721:
            ...
            with open(
                (os.path.join(root_path, processed_path) + "/%03d.npy" % i), "wb"
            ) as f:
                np.save(f, outfile)
...
def processed_to_png(root_path, processed_path, scalers_path, png_path) -> None:
    ...
    global_min = np.load(
        os.path.join(root_path, processed_path, npy_files_list[0])
    ).min()
    global_max = np.load(
        os.path.join(root_path, processed_path, npy_files_list[0])
    ).max()
    ...
    pickle.dump(
        scaler_0255_init,
        open(os.path.join(root_path, scalers_path, "scaler_0255_init.sav"), "wb"),
    )
    pickle.dump(
        scaler_0255_compr,
        open(os.path.join(root_path, scalers_path, "scaler_0255_compr.sav"), "wb"),
    )
    ...
    for i, j in enumerate(npy_files_list):
        ...
        out_ground_pic_np = scaler_0255_init.transform(
            init_pic.reshape(-1, 1)
        )  # reshape to vector
        out_ground_pic_np = out_ground_pic_np.reshape((720, 720))  # reshape to matrix

        print(
            "shape 1, MIN MAX:",
            out_ground_pic_np.shape,
            out_ground_pic_np.min(),
            out_ground_pic_np.max(),
        )
        ...
        print(
            "shape 1 compr, MIN MAX:",
            out_compr_pic_np.shape,
            out_compr_pic_np.min(),
            out_compr_pic_np.max(),
        )
...
def inverse_transform_to_txt(
    ...
    print(
        ...
        len(os.listdir(os.path.join(root_path, results_path))),
    )

    scaler_0255_init = pickle.load(
        open(os.path.join(root_path, scalers_path, "scaler_0255_init.sav"), "rb")
    )
    ...
    for file_name in results_list:
        ...
        img_ground_big = np.array(
            im.open(os.path.join(root_path, png_path, f"ground_{number}.png"))
        )  # [:,:,0]
        # img_ground_small = np.array(im.open(f'dataset_png/compr_0{NUM}.png'))#[:,:,0]
        img_upscaled_bic = np.array(
            im.open(os.path.join(root_path, png_path, f"bicubic_{number}.png"))
        )  # [:,:,0]
        img_upscaled_nn = np.array(
            im.open(os.path.join(root_path, results_path, f"compr_{number}_out.png"))
        )  # [:,:,0] # here was -- convert L !

        print(f"{file_name} -> {output_path}***.txt")

        # inverse transform and export to TXT
        np.savetxt(
            os.path.join(root_path, output_path, f"ground_big_{number}.txt"),
            scaler_0255_init.inverse_transform(img_ground_big),
            delimiter=",",
        )
        # np.savetxt(f'maps_output/ground_small_00{i}.txt', scaler_0255_compr.inverse_transform(img_ground_small), delimiter=',')
        np.savetxt(
            os.path.join(root_path, output_path, f"upscaled_bicubic_{number}.txt"),
            scaler_0255_init.inverse_transform(img_upscaled_bic),
            delimiter=",",
        )
        np.savetxt(
            os.path.join(root_path, output_path, f"upscaled_nn_{number}.txt"),
            scaler_0255_init.inverse_transform(img_upscaled_nn),
            delimiter=",",
        )
